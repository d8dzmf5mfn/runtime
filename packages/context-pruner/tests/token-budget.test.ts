...
describe('TokenBudgetManager', () => {
  ...
  it('reads files from configured repo root instead of process cwd', () => {
    const repoRoot = mkdtempSync(join(tmpdir(), 'token-budget-'));
    try {
      writeFileSync(join(repoRoot, 'tracked.ts'), 'export const tracked = true;\n');
      const tb = new TokenBudgetManager(128000, 50, repoRoot);
      const estimate = tb.estimateTokens([{ path: 'tracked.ts', score: 1, reasons: [] }]);
