...
# 信任项目后，Codex 会读取 `.codex/config.toml` 并启动 Runtime MCP
```

### 方式三：用 Claude Code 安装
...
如果只使用 MCP 工具，可复用 `.mcp.json`（Claude Code）或 `.codex/config.toml`（Codex）。两者都要求先构建 `packages/mcp-server/dist/index.js`。
...
项目级配置位于 `.codex/config.toml`（Codex 仅在信任项目后加载）：
...
```json
{
  "mcpServers": {
    "runtime": {
      ...
      "env": { "VSCODE_WORKSPACE_ROOT": "${CLAUDE_PROJECT_DIR}" }
    }
  }
}
```
...
npm install
npm run build
npm test          # 41 个测试用例（含插件回归与独立 MCP 启动验证）
```
